...
async function fetchRecentSessionsPage(offset: number, limit: number): Promise<RecentSession[]> {
  const from = offset;
...
export function useRecentSessionsInfinite(pageSize = 5) {
  return useInfiniteQuery({
    queryKey: ['recent-sessions', 'infinite', pageSize],
    initialPageParam: 0,
    staleTime: 30_000,
    queryFn: async ({ pageParam }) => fetchRecentSessionsPage(Number(pageParam ?? 0), pageSize),
